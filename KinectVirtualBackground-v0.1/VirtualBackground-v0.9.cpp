#pragma comment( linker, "/subsystem:windows /entry:mainCRTStartup" )	//隐藏终端窗口

#include "stdafx.h"
#include "VirtualBackground-v0.9.h"
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			for (int i = 0; i < BODY_COUNT; i++)
			{
				...
				{
					// 把被追踪到的人的索引存入队列
					bodyTrackedQueue.push(i);
				}
			}

			if (!bodyTrackedQueue.empty())
			{
				// 检测队列中的索引是否仍被追踪，如果没有被追踪则弹出队列
				// 目前存在的问题：如果被追踪到的人丢失，会造成追踪下一个人有延迟，原因是队列中失效的索引没有及时被清除。
				int index_bodyTracked;
				index_bodyTracked = bodyTrackedQueue.front();
				BOOLEAN tracked2 = false;
				body[index_bodyTracked]->get_IsTracked(&tracked2);

				if (tracked2)
				{
					Joint joint[JointType_Count];
					hr = body[index_bodyTracked]->GetJoints(JointType_Count, joint);
					cout << "index_bodyTracked: " << index_bodyTracked << " " << clock()  << endl;
					//if ((joint[JointType_Head].Position.Z < TRACKEDPEOPLE_MAXZ) && (joint[JointType_Head].Position.Z > TRACKEDPEOPLE_MINZ))
					if (joint[JointType_Head].Position.Z < TRACKEDPEOPLE_MAXZ)
					{
						...
						body[index_bodyTracked]->get_HandRightState(&rightHandState);
						int gTime = 200;
						int gDistance = 5;
			...
			for (int i = 0; i < colorHeight; i++)
			{
				for (int j = 0; j < colorWidth; j++)
				{
					...
					if ((depthX >= 0 && depthX < depthWidth) && (depthY >= 0 && depthY < depthHeight))
					{
						...
						if (SUCCEEDED(hr))
						{
							//if ((caspp.Z < TRACKEDPEOPLE_MAXZ) && (caspp.Z > TRACKEDPEOPLE_MINZ))
